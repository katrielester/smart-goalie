# streamlit_app.py chat_message ver
...
def append_bot_message(text):
    st.session_state["chat_thread"].append({"sender": "Assistant", "message": text})

def append_user_message(text):
    st.session_state["chat_thread"].append({"sender": "User", "message": text})
...
def run_intro():
    ...
    if "did_intro_rerun" not in st.session_state or not st.session_state["did_intro_rerun"]:
        if not any(entry["message"] == intro_message for entry in st.session_state["chat_thread"]):
            append_bot_message(intro_message)
    ...
    if st.session_state.get("chat_state") == "intro":
        if st.button("Yes, let's start!", key="start_training_btn"):
            append_user_message("Yes, let's start!")
...
def run_smart_training():
    ...
    if not all_texts_rendered:
        ...
        if current_text not in rendered_messages:
            append_bot_message(current_text)
        st.session_state["message_index"] += 1
        time.sleep(0.6)
        st.rerun()
    else:
        if step.get("buttons"):
            selected = st.radio("Choose:", step["buttons"], key=f"buttons_{st.session_state['smart_step']}")
            if st.button("Next"):
                append_user_message(selected)
...
def run_view_goals():
    if not any("Your SMART Goals:" in m["message"] for m in st.session_state["chat_thread"]):
        goals = get_goals(st.session_state["user_id"])
        if not goals:
            append_bot_message("You haven’t created any goals yet.")
        else:
            goal_msg = "**Your SMART Goals:**\n"
            for goal_id, goal_text in goals:
                goal_msg += f"**Goal:** {goal_text}\n"
                tasks = get_tasks(goal_id)
                if tasks:
                    for task_id, task_text, completed in tasks:
                        status = "✅" if completed else "⬜️"
                        goal_msg += f"{status} {task_text}\n"
                else:
                    goal_msg += "_No subtasks yet._\n"
                goal_msg += "---\n"
            append_bot_message(goal_msg)
